...
# ---------------------------------------------------------------------------
# ACP launcher: --jar must fully bypass jbang
# ---------------------------------------------------------------------------


def test_run_acp_server_with_jar_bypasses_jbang(monkeypatch, tmp_path) -> None:
    """`brokk acp --jar <path>` must launch `java -cp <path>` directly.

    With --jar provided, the launcher must not call ensure_jbang_ready, must
    not invoke a jbang binary, and must not construct any release-jar URL
    pointing at brokk-releases.
    """
    captured: dict[str, object] = {}
    dummy_jar = tmp_path / "brokk.jar"
    dummy_jar.write_text("dummy")
